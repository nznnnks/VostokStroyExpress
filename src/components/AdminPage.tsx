const sideNav = [
  ["/админка/дашборд.svg", "Дашборд", "/admin", "", true],
  ["/админка/заявки и проекты.svg", "Заявки", "/admin#requests", "1", false],
  ["/админка/заказы.svg", "Заказы", "/admin#orders", "2", false],
  ["/админка/клиент новости каталог.svg", "Клиенты", "/admin#clients", "", false],
  ["/админка/клиент новости каталог.svg", "Новости", "/admin#news", "", false],
  ["/админка/клиент новости каталог.svg", "Каталог", "/admin#catalog", "", false],
  ["/админка/заявки и проекты.svg", "Проекты", "/admin#projects", "", false],
  ["/админка/настройки.svg", "Настройки", "/admin#settings", "", false],
];
...
const events = [
  ["#AE-9021", "Skyline Residences", 'ООО "Вертикаль"', "VRF Multi-Split", "Активно", "#2ebf63", "22.10.2024", "450,000 ₽"],
  ["#AE-9018", "Nordic Museum", "Архитектурное бюро A1", "Precision Chiller", "Ожидание", "#8a6a2a", "21.10.2024", "1,200,000 ₽"],
  ["#AE-8995", "Industrial Park Beta", "Технопром групп", "HVAC Central Unit", "Требует внимания", "#b24c47", "20.10.2024", "890,500 ₽"],
  ["#AE-8992", "Green Valley Office", "Инвестдевелопмент", "Smart Air Flow", "#", "#d8d8d8", "19.10.2024", "210,000 ₽"],
];
...
export function AdminPage() {
  return (
    <main className="min-h-screen bg-[#faf8f4] text-[#111] [font-family:DM_Sans,Manrope,'Liberation_Sans',sans-serif]">
      <div className="grid min-h-screen xl:grid-cols-[420px_1fr] 2xl:grid-cols-[460px_1fr]">
        <aside className="flex min-h-screen flex-col bg-[#211d1a] text-white">
          <div className="border-b border-white/10 px-8 py-12">
            <p className="max-w-full text-[30px] italic uppercase leading-none tracking-[-0.04em] text-white 2xl:text-[34px] [font-family:'Cormorant_Garamond',serif]">
              ВОСТОКСТРОЙЭКСПЕРТ
            </p>
          ...
          <nav className="pt-6">
            {sideNav.map(([icon, label, href, badge, active]) => (
              <a
                key={label as string}
                href={href as string}
        ...
        <div className="min-w-0">
          <header className="border-b border-[#e8e3db] bg-white px-6 py-5 md:px-10">
            <div className="flex items-center justify-end gap-6">
              ...
              <span className="text-[18px]">Александр В.</span>
              <img
                src="/админка/фото профиля.png"
            ...
            <div className="mx-auto max-w-[1280px]">
              ...
              <div className="mt-8 grid gap-6 xl:grid-cols-3">
                {metrics.map(([label, value, suffix, delta, icon, bars], index) => (
                  <article key={label as string} className="border border-[#e8e3db] bg-white p-8 md:p-9">
                    ...
                    <div className="mt-12 flex min-h-[88px] items-end justify-between gap-4">
                      <div className="flex min-w-0 items-end gap-3">
                        <p className="whitespace-nowrap text-[34px] uppercase tracking-[2px] md:text-[48px] [font-family:Jaldi,'JetBrains_Mono',monospace]">{value}</p>
                        {suffix ? (
                    ...
                    <div className="mt-10">
                      {index === 0 ? (
                        <img
                          ...
                          loading="lazy"
                          decoding="async"
                          className="h-4 w-[72%] object-contain"
                        />
                      ) : (
              ...
              <section className="mt-24 border border-[#e8e3db] bg-white">
                <div className="flex items-center justify-between border-b border-[#ece8e1] px-8 py-8 md:px-12">
                ...
                {events.map(([id, project, company, type, status, color, date, amount]) => (
                  <div key={id as string} className="grid gap-4 border-b border-[#ece8e1] px-8 py-8 md:grid-cols-[1fr_1.5fr_1.2fr_1.2fr_1fr_1fr] md:items-center">
                    <div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-[18px]">{status === "#" ? "Завершено" : status}</span>
                      </div>
                    </div>
                    <div>
